...
function loadCatalogo(){
  fetch("data/catalogo.json").then(r=>r.json()).then(d=>{
    ...
    d.categorias.forEach(cat=>{
      ...
      s.innerHTML=`<h2>${cat.nombre}</h2><p>${cat.descripcion}</p><div class="productos"></div>`;
      ...
      cat.productos.forEach(p=>{
        ...
        card.innerHTML=`
          ...
          <a target="_blank" href="${WA+encodeURIComponent("Interesado en "+p.nombre)}">Comprar</a>
        `;
        g.appendChild(card);
      });
      c.appendChild(s);
...
function loadSimple(url,title){
  fetch(url).then(r=>r.json()).then(d=>{
    const c=document.getElementById("content");
    c.innerHTML=`<section class="categoria"><h2>${title}</h2><div class="productos"></div></section>`;
    const g=c.querySelector(".productos");
    (d.items||[]).forEach(i=>{
      const card=document.createElement("div");
      card.className="card";
      card.innerHTML=`<h3>${i.titulo}</h3><p>${i.descripcion}</p>
      <a target="_blank" href="${WA+encodeURIComponent("Info "+i.titulo)}">Solicitar info</a>`;
      g.appendChild(card);
    });
...
function loadMembers(){
  fetch("data/members.json").then(r=>r.json()).then(d=>{
    const c=document.getElementById("content");
    c.innerHTML=`<section class="categoria"><h2>Members</h2><div class="productos"></div></section>`;
    const g=c.querySelector(".productos");
    d.planes.forEach(p=>{
      const card=document.createElement("div");
      card.className="card";
      card.innerHTML=`<h3>${p.nombre}</h3><p>${p.detalle}</p>
      <a target="_blank" href="${WA+encodeURIComponent("Plan "+p.nombre)}">Unirme</a>`;
      g.appendChild(card);
    });
